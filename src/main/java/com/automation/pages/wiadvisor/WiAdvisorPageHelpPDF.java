...
public class WiAdvisorPageHelpPDF extends WiAdvisorFinalizeROTab {

    PDFUtils pdf = new PDFUtils();

    public String clickGetMaintMenuPDF(){
        clickMaintMenuLink();
    ...
    public String clickGetOpenVIPlinkPDF(){
        clickOpenVIPlink();
        String content =pdf.getPDFContent();
        return content;
    }
...
//		int countTotal = Integer.valueOf(StringUtils.countMatches(content, "total"));
    ...
    public void verifyPreServiceWriteUp(String pdfContent, String field1){
        if(pdfContent.contains(field1)){
            System.out.println("<====== Pre-Service Write-Up verified ======>");
        }else{
            Assert.fail("<====== Pre-service Write-Up PDF has issue, test failed ======>");
        }
    }

    public void verifyOpenVIP(String pdfContent, String field1){
        if(pdfContent.contains(field1)){
            System.out.println("<====== EVIR PDF verified ======>");
        }else{
            Assert.fail("<====== EVIR PDF has issue, test failed ======>");
        }
    }
